...
export async function POST(req: NextRequest) {
  try {
    const limit = await checkRateLimit(req, { namespace: "contact", maxRequests: 3, windowSeconds: 900 });
    ...
    const safeFirstName = escapeHtml(name.split(/\s+/)[0]);
            ...
            <div style="background: #ffffff; border: 1px solid #E2E8F0; border-radius: 12px; padding: 24px; margin-bottom: 20px;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #F0F4F8; font-size: 15px; color: #0C2D54;">${safeName}</td>
                </tr>
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #F0F4F8; font-size: 15px; color: #0C2D54;"><a href="mailto:${safeEmail}" style="color: #3D80C8; text-decoration: none;">${safeEmail}</a></td>
                </tr>
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #F0F4F8; font-size: 15px; color: #0C2D54;">${safePhone}</td>
                </tr>
                <tr>
                  ...
                  <td style="padding: 10px 0; font-size: 15px; color: #0C2D54; line-height: 1.7;">${safeMessage}</td>
                </tr>
              </table>
            </div>

            <a href="mailto:${safeEmail}" style="display: inline-block; background: #17A882; color: #ffffff; font-size: 14px; font-weight: 600; padding: 12px 24px; border-radius: 8px; text-decoration: none;">Reply to ${safeName}</a>
          </div>
      ...
      html: `
        <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
          ...
          <div style="padding: 40px; background: #F7F9FC; border: 1px solid #E2E8F0; border-top: none;">
            <h1 style="font-size: 26px; color: #0C2D54; margin: 0 0 16px; line-height: 1.2;">Thanks, ${safeFirstName}. We'll be in touch.</h1>
            ...
            <div style="background: #ffffff; border: 1px solid #E2E8F0; border-left: 4px solid #3D80C8; border-radius: 0 10px 10px 0; padding: 18px 20px; margin-bottom: 28px;">
              ...
              <div style="font-size: 14px; color: #334155; line-height: 1.7;">${safeMessage}</div>
            </div>
