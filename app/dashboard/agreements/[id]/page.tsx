...
type Finding = {
  ...
  sourceVerified?:boolean;assessment?:string;contextQuotes?:Array<{sourcePage:number|null;contractText:string;sourceVerified:boolean}>;
};
...
function FindingCard({finding,free=false}:{finding:Finding;free?:boolean}){
  return <article className="mt-5 rounded-2xl border-2 border-teal/25 bg-white p-6"><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-sans text-[10px] font-semibold uppercase tracking-wider text-teal">{free?"Your free agreement finding":`${finding.risk} attention`}</div>{finding.sourcePage&&<div className="font-sans text-xs text-gray-500">Page {finding.sourcePage}</div>}</div><h3 className="mt-2 font-serif text-2xl text-navy">{finding.title}</h3><blockquote className="mt-4 border-l-4 border-blue bg-blue-pale/30 p-4 font-sans text-sm italic leading-6 text-gray-700">“{finding.contractText}”</blockquote>{(finding.sourceVerified!==true||finding.contextQuotes?.some(quote=>!quote.sourceVerified))&&<p className="mt-2 font-sans text-sm text-amber-700">AI-transcribed text: compare with the original. An independently verified source is required before an email draft is available.</p>}{finding.contextQuotes?.map((quote,index)=><blockquote key={index} className="mt-3 border-l-4 border-teal bg-teal-light/30 p-4 font-sans text-sm leading-6 text-gray-700"><strong>Related protection{quote.sourcePage ? ` — page ${quote.sourcePage}` : ""}:</strong> {quote.contractText}</blockquote>)}<div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-off-white p-4"><div className="font-sans text-[10px] font-semibold uppercase tracking-wider text-gray-500">What it means</div><p className="mt-1 font-sans text-sm leading-6 text-gray-700">{finding.plainEnglish}</p></div><div className="rounded-xl bg-teal-light/50 p-4"><div className="font-sans text-[10px] font-semibold uppercase tracking-wider text-teal">What to do now</div><p className="mt-1 font-sans text-sm leading-6 text-navy">{finding.recommendedAction}</p></div></div>{finding.emailTemplate?<EmailTemplateBox template={finding.emailTemplate}/>:free&&finding.sourceVerified===true&&<div className="mt-4 rounded-xl bg-navy p-4 font-sans text-sm text-white/75">The tailored vendor email for this exact clause is included after purchasing the agreement review or joining Pro.</div>}</article>;
}
