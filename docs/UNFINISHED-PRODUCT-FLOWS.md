...
Status: Staging implementation added September 16, 2026. The billing migrations were applied to the explicitly approved Supabase project `xrchncayomnwcnphrwhx`. The application is deployed only as a [Vercel preview](https://my-contract-doctors-git-codex-billing-rel-a1b0c0-osc-web-design.vercel.app/) from branch `codex/billing-release-pass`; the production site has not been updated. No payment provider or real charges are connected.
